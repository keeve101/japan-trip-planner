...
with sync_playwright() as p:
    ...
    for label in labels:
        ...
        try:
            ...
            continue
        data=page.evaluate("""(label)=>({label,imgs:[...document.querySelectorAll('.tab.active img')].map((im,i)=>({src:im.currentSrc||im.src,alt:im.alt,loaded:im.complete&&im.naturalWidth>0,card:im.closest('.rec-card')?.querySelector('h3')?.innerText||null,hero:!!im.closest('.hero-photo,.dest-head')}),)} )""",label)
        records.append(data)
    out=defaultdict(lambda:{'uses':0,'tabs':set(),'cards':[],'loaded':0,'missing':0,'urls':set()})
    for tab in records:
        for im in tab['imgs']:
            src=im['src']
    ...
    result={k:{'uses':v['uses'],'tabs':sorted(v['tabs']),'cards':v['cards'],'loaded':v['loaded'],'missing':v['missing'],'urls':sorted(v['urls'])} for k,v in sorted(out.items(),key=lambda kv:-kv[1]['uses'])}
